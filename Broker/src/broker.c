...
int main(void) {
	...
	if(signal(SIGPIPE, SIG_IGN) == SIG_ERR)
		log_info(logger,"PADRE NUESTRO QUE ESTAS EN EL CIELO, SANTIFICADO SEA TU NOMBRE. VENGA A NOSOTROS TU REINO");
	init_broker_server();
	return EXIT_SUCCESS;
}
		...
		switch(message->head){
			case NEW_POKEMON:{
				...
				while(indice<sizeCola){
					...
					pthread_mutex_unlock(mutexSuscriptoresNew);
					int socketSuscriptor = mensajeAEnviar->socketEnvio;
					...
					bool mismoSocket(void* sus){
						...
						return suscriptorCasteado->socket == socketSuscriptor;
					}
			...
			case CAUGHT_POKEMON:{
				...
				pthread_mutex_lock(mutexIDsCorrelativosCaught);
				uint32_t* busqueda_id_c = buscarIdCorrelativo(idsCorrelativosCaught,mensaje.idCorrelativo);
				pthread_mutex_unlock(mutexIDsCorrelativosCaught);
			...
			case LOCALIZED_POKEMON:{
				...
				pthread_mutex_lock(mutexIDsCorrelativosLocalized);
				uint32_t* busqueda_id_c = buscarIdCorrelativo(idsCorrelativosLocalized,mensaje.idCorrelativo);
				pthread_mutex_unlock(mutexIDsCorrelativosLocalized);
			...
			while(index<sizeCola){
				...
				if(particion != NULL){
					...
					if (suscriptorEncontrado == NULL){
						...
						size_t sizeCaught = sizeof(cpEnviar.id_mensaje)+sizeof(cpEnviar.idCorrelativo) + particion->tamanioMensaje;
